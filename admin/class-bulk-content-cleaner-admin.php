...
class Bulk_Content_Cleaner_Admin {
    ...
    public function ajax_delete_posts() {
        ...
        $log          = array();
        $deleted      = 0;
        $deleted_media = 0; // Track media deletions separately
        $errors       = 0;
        $skipped      = 0;
        $has_more     = false;
        $next_offset  = 0; // Always 0 because we are deleting items from the top
        ...
        if ( $delete_posts ) {
            ...
            $args = array(
                ...
                'offset'         => 0, // Force offset 0 to avoid skipping items as they are deleted
                'fields'         => 'ids',
                'orderby'        => 'ID',
                'order'          => 'ASC',
                'no_found_rows'  => false,
            ...
            if ( ! empty( $post_ids ) ) {
                ...
                $next_offset = 0; // Always start from top
                $has_more    = $total_found > count( $post_ids ); // If we found more than we processed, there are more
